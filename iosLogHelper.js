function parseIosLine(line, state) {
    ...
    if (state.collecting && line.includes("conferenceWillJoin received with data")) {
        let jsonStr = line;
        while (state.i + 1 < logLines.length && !logLines[state.i + 1].includes("}")) {
            jsonStr += logLines[++state.i];
        }
        jsonStr += logLines[++state.i];
    ...
    if (state.collecting && line.includes("[CallModule][StateMachine] - Call State Changed from ended")) {
    ...
    if (state.collecting && line.includes("conferenceUpdateParticipant received with data")) {
        let jsonStr = line;

        while (state.i + 1 < logLines.length && !logLines[state.i + 1].trim().endsWith("}")) {
            jsonStr += logLines[++state.i].trim();
        }
        jsonStr += logLines[++state.i].trim();
    ...
    // ✅ SIGNALIZATIONS: VCIT örneği
    if (state.collecting && line.includes("VOIP | Push Message Type : VCIT")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        if (!state.currentCall.signalingEvents.VCIT) {
            state.currentCall.signalingEvents.VCIT = [];
        }

        state.currentCall.signalingEvents.VCIT.push({
            timestamp,
            message: line.trim()
        });
    ...
    // ✅ SIGNALIZATIONS: GlareCondition log entry
    if (state.collecting && line.includes("[CallModule][Manager][GlareCondition] - Total Sessions Count")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        if (!state.currentCall.signalingEvents.glareConditionSessionCount) {
            state.currentCall.signalingEvents.glareConditionSessionCount = [];
        }
        ...
        state.currentCall.signalingEvents.glareConditionSessionCount.push(entry);
...
if (state.collecting && line.includes("[CallModule][StateMachine] - Call Event Fired:")) {
    const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
    const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";
    ...
    if (
        ...
        if (!state.currentCall.signalingEvents.callStateChanges) {
            state.currentCall.signalingEvents.callStateChanges = [];
        }
        ...
        state.currentCall.signalingEvents.callStateChanges.push(entry);
    ...
    if (state.collecting && line.includes("[Connection]: State is updated. State: Authenticated")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";
    ...
    if (state.collecting && line.includes("[CallModule][Message] - Initiate message received")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";
    ...
    if (state.collecting && line.includes("[CallModule][Message] - Info message sent to:")) {
    ...
    if (state.collecting && line.includes("Call State Changed from ringing to willJoin")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        state.currentCall.signalingEvents.ringingToWillJoin = {
            timestamp,
            message: line.trim()
        };
    ...
    if (state.collecting && line.includes("[CallModule][CallKitProxy] - CXAnswerCallAction")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        state.currentCall.signalingEvents.cxAnswerCallAction = {
            timestamp,
    ...
    if (
        ...
        state.currentCall.signalingEvents.callKitDidActivate = {
            timestamp,
            message: line.trim()
        };

        console.log("🎧 CallKitProxy didActivate Logged:", state.currentCall.signalingEvents.callKitDidActivate);
    }
    ...
    if (
        ...
        state.currentCall.signalingEvents.callKitEndCall = {
            timestamp,
            message: line.trim()
        };

        console.log("📴 CallKitProxy CXEndCallAction Logged:", state.currentCall.signalingEvents.callKitEndCall);
    }
    ...
    if (
        ...
        state.currentCall.signalingEvents.callStateEnded = {
            timestamp,
            message: line.trim()
        };

        console.log("🛑 Call State Changed to ended Logged:", state.currentCall.signalingEvents.callStateEnded);
    }
    ...
    if (state.collecting && line.includes("[CallModule][Message] - Terminate message sent to")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";
...
if (state.collecting && line.includes("[VoIP] - MaxVersion: 0.0.0 - Drop: false - AppVersion:")) {
    const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
    const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

    if (!state.currentCall.signalingEvents.maxVersionEvents) {
        state.currentCall.signalingEvents.maxVersionEvents = [];
    }
    ...
    state.currentCall.signalingEvents.maxVersionEvents.push(entry);
    ...
    if (state.collecting && line.includes("[CallModule][CallKitProxy] - request transaction with Action:")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        if (!state.currentCall.signalingEvents.callKitRequestTransaction) {
            state.currentCall.signalingEvents.callKitRequestTransaction = [];
        }
        ...
        state.currentCall.signalingEvents.callKitRequestTransaction.push(entry);

        console.log("📞 CallKit Transaction Request Logged:", entry);
    }

    // ✅ Jitsi token request mesajını yakala
    if (state.collecting && line.includes("[CallModule][Message] - Token request message sent.")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";
    ...
    if (state.collecting && line.includes("[CallModule][Timer] -") && line.includes("timer")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";
    ...
    if (state.collecting && line.includes("[CallModule][Message] - Token message received")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        if (!state.currentCall.signalingEvents.tokenReceived) {
            state.currentCall.signalingEvents.tokenReceived = [];
        }
        ...
        state.currentCall.signalingEvents.tokenReceived.push(entry);
    ...
    if (state.collecting && line.includes("[CallModule][Storage] - Inserting new session for outgoing call")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        if (!state.currentCall.signalingEvents.outgoingSessionInserted) {
            state.currentCall.signalingEvents.outgoingSessionInserted = [];
        }
        ...
        state.currentCall.signalingEvents.outgoingSessionInserted.push(entry);

        console.log("📤 Outgoing Session Inserted Logged:", entry);
    }

    // ✅ "[CallModule][Message] - Initiate message sent to:" bloklarını al (çok satırlı)
    if (state.collecting && line.includes("[CallModule][Message] - Initiate message sent to:")) {
    ...
    if (state.collecting && line.includes("[CallModule][Message] - Terminate message parsing succeded with from:")) {
    ...
    if (state.collecting && line.includes("Call State Changed from ended to sessionEnded")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        if (!state.currentCall.signalingEvents.sessionEndedStates) {
            state.currentCall.signalingEvents.sessionEndedStates = [];
        }
        ...
        state.currentCall.signalingEvents.sessionEndedStates.push(entry);
    ...
    if (state.collecting && line.includes("Call State Changed from willJoin to joined")) {
        const timestampMatch = line.match(/^(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}:\d{3})/);
        const timestamp = timestampMatch ? timestampMatch[1] : "Unknown Timestamp";

        if (!state.currentCall.signalingEvents.willJoinToJoinedStates) {
            state.currentCall.signalingEvents.willJoinToJoinedStates = [];
        }
        ...
        state.currentCall.signalingEvents.willJoinToJoinedStates.push(entry);
...
window.parseIosLine = parseIosLine;